...
var ActiveScan = (function() {
	...
	function showReqDialog(tabId, uri, method, body) {
		Promise.all([checkIsRunning(tabId), self.tools.scope.isInScope(uri)])
			.then(results => {
				var isRunning = results[0];
				var isInScope = results[1];
				var config = {};
			...
			.then(config => {
				return utils.messageFrame(tabId, "display", {action:"showDialog", config:config})
			})
			.catch(utils.errorHandler);
	}
